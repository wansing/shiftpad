...
func GetSystemLocation() *time.Location {
	...
	for _, cl := range canonicalLocations {
		if strings.HasSuffix(path, cl) {
			loc, err := time.LoadLocation(cl)
			if err != nil {
				return time.UTC
			}
			return loc
...
func Locations(loc string) []string {
	if slices.Contains(canonicalLocations, loc) {
		return canonicalLocations
	}
	locations := append(canonicalLocations, loc)
